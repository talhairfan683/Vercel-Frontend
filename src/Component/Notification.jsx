...
export default function Notification() {
  ...
  return (
    ...
    <aside className="w-32 bg-[#16131d] border-r border-gray-800/50 flex flex-col items-center py-6 gap-8">
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto ml-32">
          ...
          {notifications.length === 0 ? (
            ...
            <div className="space-y-4">
              {notifications.map((notification) => (
                <div 
                  key={notification.id}
                  className="bg-[#1e1b2e] rounded-2xl p-6 border border-gray-800/50 hover:border-gray-700/50 transition"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="text-white text-xl font-bold mb-2">{notification.title}</h3>
                    ...
                    <div className="ml-4">
                      {notification.type === 'rejected' && (
                        <button 
                          ...
                          className="bg-[#e95260] text-white px-5 py-2.5 rounded-lg font-semibold transition text-sm whitespace-nowrap"
                        >
                          View comment
                        </button>
                      )}
